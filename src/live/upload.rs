...
/// Take a stored file, or `None` if unknown / expired.
pub fn take(id: &str) -> Option<serde_json::Value> {
    let mut map = store().lock().unwrap_or_else(|e| e.into_inner());
    prune_locked(&mut map);
...
pub fn hydrate_event_params(params: &mut serde_json::Value) {
    hydrate_one(params);
...
fn hydrate_one(value: &mut serde_json::Value) {
...
mod tests {
    ...
    fn put_then_take_hydrates_base64_data() {
        let meta = put("avatar", "x.png", "image/png", b"\x89PNG".to_vec()).unwrap();
        ...
        hydrate_event_params(&mut params);
        ...
        hydrate_event_params(&mut again);
    ...
    fn consume_round_trips_original_bytes() {
        ...
        let meta = put("doc", "note.txt", "text/plain", bytes.to_vec()).unwrap();
    ...
    fn oversized_put_is_refused() {
        ...
        assert!(put("f", "big.bin", "application/octet-stream", big).is_err());
    }
}
