...
def oidc_json(body_hash, status)
  return {
    "status": status,
    "headers": {
...
def oidc_token_error(code, description, status)
  body = {"error": code}
...
def oidc_authorize_error(redirect_uri, code, description, state)
...
def oidc_id_token(user, client_id, scopes, nonce, auth_time, access_token)
...
def oidc_verify_access_token(token)
