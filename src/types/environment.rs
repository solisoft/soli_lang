...
impl TypeEnvironment {
    ...
    fn register_builtin_classes(&mut self) {
        ...
        // Paseto class — PASETO v4 tokens (feature `paseto`, on by default).
        // Keys and tokens are PASERK strings (`k4.local.…`, `k4.secret.…`,
        // `k4.public.…`), so the key helpers and the two producers return
        // String; the three consumers return the claims as a Hash. Params are
        // `Any` for the same reason as Crypto's: the methods take (claims, key)
        // or (token, key) plus an optional options hash, and spelling that out
        // here would only add arity false-positives.
        #[cfg(feature = "paseto")]
        {
