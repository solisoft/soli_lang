...
impl Interpreter {
    ...
    pub(crate) fn evaluate_call(
        ...
        if matches!(
            ...
            // Model lifecycle callbacks (before_save/before_delete/etc.) are no
            // longer intercepted here — they run inside the native model methods
            // (see `builtins/model/core.rs` + `callbacks::run_lifecycle_callbacks`)
            // so they fire identically under the executor and the bytecode VM.
