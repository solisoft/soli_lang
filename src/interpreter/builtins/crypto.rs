...
pub fn register_crypto_builtins(env: &mut Environment) {
    ...
    crypto_static_methods.insert(
        ...
        Rc::new(NativeFunction::new("Crypto.sha256", Some(1), |args| {
            let data = match &args[0] {
                ...
                other => return Err(format!("Crypto.sha256() expects string, got {}", other.type_name())),
            };
    ...
    crypto_static_methods.insert(
        ...
        Rc::new(NativeFunction::new("Crypto.sha512", Some(1), |args| {
            let data = match &args[0] {
                Value::String(s) => s.clone(),
                other => return Err(format!("Crypto.sha512() expects string, got {}", other.type_name())),
            };
    ...
    crypto_static_methods.insert(
        ...
        Rc::new(NativeFunction::new("Crypto.md5", Some(1), |args| {
            let data = match &args[0] {
                Value::String(s) => s.clone(),
                other => return Err(format!("Crypto.md5() expects string, got {}", other.type_name())),
            };
    ...
    crypto_static_methods.insert(
        ...
        Rc::new(NativeFunction::new("Crypto.hmac", Some(2), |args| {
            let message = match &args[0] {
                Value::String(s) => s.clone(),
                other => return Err(format!("Crypto.hmac() expects string message, got {}", other.type_name())),
            };
            let key = match &args[1] {
                Value::String(s) => s.clone(),
                other => return Err(format!("Crypto.hmac() expects string key, got {}", other.type_name())),
            };
    ...
    crypto_static_methods.insert(
        ...
        Rc::new(NativeFunction::new("Crypto.argon2_hash", Some(1), |args| {
            let password = match &args[0] {
                ...
                other => return Err(format!("Crypto.argon2_hash() expects string, got {}", other.type_name())),
            };
    ...
    crypto_static_methods.insert(
        ...
        Rc::new(NativeFunction::new("Crypto.argon2_verify", Some(2), |args| {
            let password = match &args[0] {
                Value::String(s) => s.as_bytes().to_vec(),
                other => return Err(format!("Crypto.argon2_verify() expects string password, got {}", other.type_name())),
            };
            let hash = match &args[1] {
                Value::String(s) => s.clone(),
                other => return Err(format!("Crypto.argon2_verify() expects string hash, got {}", other.type_name())),
            };
    ...
    crypto_static_methods.insert(
        ...
        Rc::new(NativeFunction::new("Crypto.password_hash", Some(1), |args| {
            let password = match &args[0] {
                Value::String(s) => s.as_bytes().to_vec(),
                other => return Err(format!("Crypto.password_hash() expects string, got {}", other.type_name())),
            };
            let hash = do_argon2_hash(&password)?;
            Ok(Value::String(hash))
        })),
    ...
    crypto_static_methods.insert(
        ...
        Rc::new(NativeFunction::new("Crypto.password_verify", Some(2), |args| {
            let password = match &args[0] {
                Value::String(s) => s.as_bytes().to_vec(),
                other => return Err(format!("Crypto.password_verify() expects string password, got {}", other.type_name())),
            };
            let hash = match &args[1] {
                Value::String(s) => s.clone(),
                other => return Err(format!("Crypto.password_verify() expects string hash, got {}", other.type_name())),
            };
            let is_valid = do_argon2_verify(&password, &hash)?;
            Ok(Value::Bool(is_valid))
        })),
    ...
    crypto_static_methods.insert(
        ...
        Rc::new(NativeFunction::new("Crypto.x25519_shared_secret", Some(2), |args| {
            ...
            if private_bytes.len() != X25519_PRIVATE_KEY_LENGTH {
                return Err(format!(
                    ...
                    X25519_PRIVATE_KEY_LENGTH, private_bytes.len()
                ));
            }
            ...
            if public_bytes.len() != X25519_PUBLIC_KEY_LENGTH {
                return Err(format!(
                    ...
                    X25519_PUBLIC_KEY_LENGTH, public_bytes.len()
                ));
            }
            let mut private_key = [0u8; X25519_PRIVATE_KEY_LENGTH];
    ...
    crypto_static_methods.insert(
        ...
        Rc::new(NativeFunction::new("Crypto.x25519_public_key", Some(1), |args| {
            let private_bytes = value_to_bytes(&args[0])
    ...
    crypto_static_methods.insert(
        ...
        Rc::new(NativeFunction::new("Crypto.base64_encode", Some(1), |args| {
            let data = match &args[0] {
                Value::String(s) => s.clone(),
                other => return Err(format!("Crypto.base64_encode() expects string, got {}", other.type_name())),
            };
    ...
    crypto_static_methods.insert(
        ...
        Rc::new(NativeFunction::new("Crypto.base64_decode", Some(1), |args| {
            let data = match &args[0] {
                Value::String(s) => s.clone(),
                other => return Err(format!("Crypto.base64_decode() expects string, got {}", other.type_name())),
            };
    ...
    env.define(
        ...
        Value::NativeFunction(NativeFunction::new("sha256", Some(1), |args| {
            let data = match &args[0] {
                Value::String(s) => s.clone(),
                other => return Err(format!("sha256() expects string, got {}", other.type_name())),
            };
            Ok(Value::String(do_sha256(&data)))
        })),
    );
    ...
    env.define(
        ...
        Value::NativeFunction(NativeFunction::new("sha512", Some(1), |args| {
            let data = match &args[0] {
                Value::String(s) => s.clone(),
                other => return Err(format!("sha512() expects string, got {}", other.type_name())),
            };
            Ok(Value::String(do_sha512(&data)))
        })),
    );
    ...
    env.define(
        ...
        Value::NativeFunction(NativeFunction::new("hmac", Some(2), |args| {
            let message = match &args[0] {
                Value::String(s) => s.clone(),
                other => return Err(format!("hmac() expects string message, got {}", other.type_name())),
            };
            let key = match &args[1] {
                Value::String(s) => s.clone(),
                other => return Err(format!("hmac() expects string key, got {}", other.type_name())),
            };
            let result = do_hmac_sha256(&message, &key)?;
            Ok(Value::String(result))
        })),
    ...
    env.define(
        ...
        Value::NativeFunction(NativeFunction::new("base64_encode", Some(1), |args| {
            let data = match &args[0] {
                Value::String(s) => s.clone(),
                other => return Err(format!("base64_encode() expects string, got {}", other.type_name())),
            };
            Ok(Value::String(do_base64_encode(&data)))
        })),
    );
    ...
    env.define(
        ...
        Value::NativeFunction(NativeFunction::new("base64_decode", Some(1), |args| {
            let data = match &args[0] {
                Value::String(s) => s.clone(),
                other => return Err(format!("base64_decode() expects string, got {}", other.type_name())),
            };
            let result = do_base64_decode(&data)?;
            Ok(Value::String(result))
        })),
    ...
    env.define(
        ...
        Value::NativeFunction(NativeFunction::new("argon2_hash", Some(1), |args| {
            let password = match &args[0] {
                Value::String(s) => s.as_bytes().to_vec(),
                other => return Err(format!("argon2_hash() expects string, got {}", other.type_name())),
            };
            let hash = do_argon2_hash(&password)?;
            Ok(Value::String(hash))
        })),
    ...
    env.define(
        ...
        Value::NativeFunction(NativeFunction::new("argon2_verify", Some(2), |args| {
            let password = match &args[0] {
                Value::String(s) => s.as_bytes().to_vec(),
                other => return Err(format!("argon2_verify() expects string password, got {}", other.type_name())),
            };
            let hash = match &args[1] {
                Value::String(s) => s.clone(),
                other => return Err(format!("argon2_verify() expects string hash, got {}", other.type_name())),
            };
            let is_valid = do_argon2_verify(&password, &hash)?;
            Ok(Value::Bool(is_valid))
        })),
    ...
    env.define(
        ...
        Value::NativeFunction(NativeFunction::new("password_hash", Some(1), |args| {
            let password = match &args[0] {
                Value::String(s) => s.as_bytes().to_vec(),
                other => return Err(format!("password_hash() expects string, got {}", other.type_name())),
            };
            let hash = do_argon2_hash(&password)?;
            Ok(Value::String(hash))
        })),
    ...
    env.define(
        ...
        Value::NativeFunction(NativeFunction::new("password_verify", Some(2), |args| {
            let password = match &args[0] {
                Value::String(s) => s.as_bytes().to_vec(),
                other => return Err(format!("password_verify() expects string password, got {}", other.type_name())),
            };
            let hash = match &args[1] {
                Value::String(s) => s.clone(),
                other => return Err(format!("password_verify() expects string hash, got {}", other.type_name())),
            };
            let is_valid = do_argon2_verify(&password, &hash)?;
            Ok(Value::Bool(is_valid))
        })),
    ...
    env.define(
        "x25519_shared_secret".to_string(),
        Value::NativeFunction(NativeFunction::new("x25519_shared_secret", Some(2), |args| {
            let private_bytes = value_to_bytes(&args[0])
    ...
    env.define(
        ...
        Value::NativeFunction(NativeFunction::new("x25519_public_key", Some(1), |args| {
            let private_bytes = value_to_bytes(&args[0])
                .map_err(|e| format!("x25519_public_key(): {}", e))?;
            if private_bytes.len() != X25519_PRIVATE_KEY_LENGTH {
                return Err(format!(
