...
impl SessionStore for SolidbSessionStore {
    ...
    /// process's first connection to `SOLI_SOLIDB_HOST` mid-request — a cold
    /// connect whose hyper driver task stalls to the client's total timeout
    /// (~10s) before recovering. A read-only `ping` (`RETURN 1`) here anchors a
    /// live pooled connection on the long-lived runtime. Read-only: never
    /// touches the request/GC counter or any session document. Best-effort —
    /// a transiently-unreachable session DB just logs and is retried lazily.
    fn warm(&self) {
        let client = match self.create_client() {
            Ok(client) => client,
            Err(e) => {
                eprintln!("Session store warmup: failed to build SoliDB client: {}", e);
                return;
            }
        };

        // Drive the warmup on the server's long-lived runtime: spawn the
        // request as a task (so its hyper connection driver is owned by, and
        // polled on, the multi-thread runtime) and block on the JoinHandle.
        // This anchors a live pooled connection before the first request,
        // instead of letting `ensure_session` open the process's first SoliDB
        // connection mid-request and stall to the client timeout.
        eprintln!(
            "{} Session store warmup (SoliDB): starting…",
            crate::serve::log_timestamp()
        );
        match crate::serve::get_tokio_handle() {
            Some(handle) => {
                // Detached: spawn the warmup on the long-lived runtime and let
                // it report its own outcome. We do NOT block boot on it — a slow
                // or unreachable session DB must not delay the server coming up.
                let warmup = client.warmup_future();
                handle.spawn(async move {
                    let started = std::time::Instant::now();
                    let elapsed = || started.elapsed().as_secs_f64() * 1000.0;
                    match warmup.await {
                        Ok(()) => eprintln!(
                            "{} Session store warmup (SoliDB): ok ({:.1}ms)",
                            crate::serve::log_timestamp(),
                            elapsed()
                        ),
                        Err(e) => eprintln!(
                            "{} Session store warmup (SoliDB) failed after {:.1}ms: {}",
                            crate::serve::log_timestamp(),
                            elapsed(),
                            e
                        ),
            ...
            // No server runtime in context (e.g. tests / non-serve callers):
            // fall back to a plain synchronous ping via the shared block_on.
            None => {
                let started = std::time::Instant::now();
