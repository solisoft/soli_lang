...
thread_local! {
    /// The serve-mode builtins registry for this thread, built once and shared
    /// by every environment on it.
    ///
    /// `register_builtins` builds roughly five hundred bindings, about a
    /// hundred of which are `Class` values carrying six `HashMap`s apiece.
    /// None of it depends on the application. A worker thread used to build
    /// that whole registry three times over — once for the interpreter's
    /// globals, once for the template engine's, once as the closure of the
    /// view helpers — and then keep all three alive for the life of the
    /// process, which is why baseline RSS rose with `SOLI_WORKERS` rather than
    /// staying flat.
    ///
    /// Nothing mutates the registry after construction, so one copy serves all
    /// three as their *enclosing* scope: each consumer keeps its own child
    /// `Environment` for what it adds on top, and name resolution falls
    /// through to this one. A child also shadows correctly — a binding defined
    /// in the child wins over the same name here, which is the ordering the
    /// separate registries had.
    ///
    /// Test-only builtins are deliberately absent. They belong to
    /// `Interpreter::new`, the `soli run` / `soli test` path, which owns its
    /// environment outright and does not share; leaking names like `visit` or
    /// `click` into a served app is a hazard `register_builtins` already
    /// guards against.
    static SERVE_BUILTINS: RefCell<Option<Rc<RefCell<Environment>>>> =
        const { RefCell::new(None) };
}

/// This thread's shared serve-mode builtins registry, built on first use.
///
/// Callers should reach for [`child_of_serve_builtins`] instead unless they
/// genuinely need to register *into* the shared scope — anything defined here
/// is visible to every interpreter, template render and view helper on the
/// thread.
pub fn serve_builtins_root() -> Rc<RefCell<Environment>> {
    SERVE_BUILTINS.with(|cell| {
        if let Some(env) = cell.borrow().as_ref() {
            return env.clone();
        }
        // Built outside the borrow: registering the Retry class evaluates Soli
        // against this same thread-local, and re-entering it while the
        // `RefCell` is held would panic.
        let mut env = Environment::with_builtins_capacity();
        register_builtins(&mut env, false);
        let root = Rc::new(RefCell::new(env));
        // Retry is evaluated Soli rather than a native, so it registers against
        // the `Rc`. Doing it here rather than in each consumer means the class
        // is parsed once per thread instead of once per environment.
...
/// A fresh, empty scope enclosed by this thread's shared builtins registry.
///
/// Resolution falls through to the shared scope, so the result behaves like a
/// full builtins environment while owning none of it.
pub fn child_of_serve_builtins() -> Rc<RefCell<Environment>> {
...
pub fn register_builtins(env: &mut Environment, include_test_builtins: bool) {
    ...
    // SQL migration DDL helpers (create_table / drop_table) for the SQL adapters.
