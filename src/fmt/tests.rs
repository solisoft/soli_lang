...
#[test]
fn comment_only_catch_body_stays_inside_the_block() {
    // Regression: a `rescue` whose entire body is a comment lost it — with no
    // statement to flush before, the comment escaped past the `end` and read as
    // documenting whatever followed. Idempotency matters as much as placement:
    // the `catch` keyword's line is written by the caller, so it must be
    // recorded or the comment gains a blank line above it on the second pass.
    let src = "def up(db)\n  begin\n    db.create_collection(\"events\")\n  rescue\n    # already exists\n  end\nend\n";
    let expected = "def up(db)\n  try\n    db.create_collection(\"events\")\n  catch\n    # already exists\n  end\nend\n";
    assert_fmt(src, expected);
    assert_idempotent(src);
}

#[test]
fn blank_line_under_a_file_header_comment_survives() {
    // Regression: the paragraph check measures from the previous statement, so
    // with no previous statement it never ran and the blank under a file header
    // was deleted — while an identical gap further down the file was kept.
    let src = "# Migration: create_users\n# Generated by: soli\n\ndef up(db)\n  x = 1\nend\n";
    assert_fmt(src, src);
    assert_idempotent(src);
}

#[test]
fn no_blank_line_added_under_an_attached_header_comment() {
    // The mirror case: no gap in the source means no blank in the output, or
    // `# soli-lint-disable-next-line` would drift off its target.
    let src = "# soli-lint-disable-next-line naming/snake-case\nlet badName = 1\n";
    assert_fmt(src, src);
    assert_idempotent(src);
}
