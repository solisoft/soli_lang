...
<span class=\"__solidev_replay\" data-req-id=\"{rid}\" title=\"replay this request server-side\" style=\"flex:0 0 auto;color:#8b949e;cursor:pointer;user-select:none;\">\u{21bb}</span>\
    ...
    // Tools panel: the dev-only galleries, which were previously reachable only
    // by typing their URL. Lives OUTSIDE `#__solidev_panels` (beside the
    // requests panel) because clicking a request row replaces that container's
    // innerHTML to retarget the per-request panels — a tools panel inside would
    // vanish on the first drill-down. Links open in a new tab so the page under
    // inspection isn't navigated away from.
    let inbox_count = crate::interpreter::builtins::mail_outbox::count();
    let inbox_badge = if inbox_count == 0 {
        String::new()
    } else {
        format!(" <span style=\"color:#b8e986;\">({})</span>", inbox_count)
    };
    let tools_panel = format!(
        "<div id=\"__solidev_tools\" class=\"__solidev_panel\" style=\"display:none;border-top:1px solid #30363d;background:#08090b;padding:0.5rem 0.75rem;\">\
<div style=\"margin-bottom:0.5rem;font-size:10px;color:#8b949e;letter-spacing:0.08em;\">DEV TOOLS \u{b7} dev-only routes \u{b7} open in a new tab</div>\
<div style=\"display:grid;grid-template-columns:repeat(auto-fill,minmax(230px,1fr));gap:0.375rem;font-size:11px;\">\
{links}\
</div>\
</div>",
        links = [
...
{name}{badge} <span style=\"color:#8b949e;\">\u{2014} {note}</span></a>"
        ))
        .collect::<String>(),
    );
...
<span style=\"color:#30363d;\">|</span>\
<button type=\"button\" id=\"__solidev_tb\" class=\"__solidev_mob\" title=\"click for the dev tools: mail inbox, mailer + component previews, database browser\" style=\"padding:0 0.25rem;border-radius:0.25rem;color:#c9d1d9;font:inherit;cursor:pointer;border:none;background:transparent;\"><svg class=\"__solidev_icon\" width=\"14\" height=\"14\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z\"/></svg><span class=\"__solidev_label\">tools</span>{inbox_badge}</button>\
</div>\
</div>{requests_panel}{tools_panel}<div id=\"__solidev_panels\">{warnings_panel}{breakdown_panel}{queries_panel}{http_panel}{kv_panel}{flame_panel}</div></aside>\
...
mod tests {
    ...
    #[test]
    fn tools_panel_sits_outside_the_swappable_panel_container() {
        // Clicking a request row replaces `#__solidev_panels`' innerHTML to
        // retarget the per-request panels; the tools panel must precede it or
        // it would be wiped on the first drill-down.
        let out = inject_dev_bar("<html><body></body></html>", &ctx("GET", "/"));
        let tools = out.find("id=\"__solidev_tools\"").expect("tools panel");
        let panels = out
            .find("id=\"__solidev_panels\"")
            .expect("panels container");
        assert!(
            tools < panels,
            "tools panel must render before the container"
        );
    }
