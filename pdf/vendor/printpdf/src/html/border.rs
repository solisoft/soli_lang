...
use crate::{Color, LineCapStyle, LineDashPattern, Mm, Op, Pt, Rgb};
...
pub fn extract_border_styles(styles: &StyleBorderStyles) -> BorderStyles {
    ...
    let convert_top = |s: Option<
        azul_css::css::CssPropertyValue<azul_css::props::style::StyleBorderTopStyle>,
    >|
     -> BorderStyleType {
    ...
    let convert_right = |s: Option<
        azul_css::css::CssPropertyValue<azul_css::props::style::StyleBorderRightStyle>,
    >|
     -> BorderStyleType {
        s.and_then(|v| v.get_property().cloned())
            .map(|v| match v.inner {
    ...
    let convert_bottom = |s: Option<
        azul_css::css::CssPropertyValue<azul_css::props::style::StyleBorderBottomStyle>,
    >|
     -> BorderStyleType {
        s.and_then(|v| v.get_property().cloned())
            .map(|v| match v.inner {
    ...
    let convert_left = |s: Option<
        azul_css::css::CssPropertyValue<azul_css::props::style::StyleBorderLeftStyle>,
    >|
     -> BorderStyleType {
        s.and_then(|v| v.get_property().cloned())
            .map(|v| match v.inner {
...
pub fn extract_border_radii(border_radius: &StyleBorderRadius) -> BorderRadii {
    let tl = border_radius
        .top_left
        .to_pixels_internal(0.0, DEFAULT_FONT_SIZE);
    let tr = border_radius
        .top_right
        .to_pixels_internal(0.0, DEFAULT_FONT_SIZE);
...
pub fn create_rounded_rect_path_with_margins(
    ...
    create_rounded_rect_path_with_margins_internal(
        x,
        y,
        width,
        height,
...
pub fn render_border(ops: &mut Vec<Op>, config: &BorderConfig) {
    ...
    let all_same = widths.top == widths.right
