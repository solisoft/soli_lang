...
- Top-level names are flat (`name="title"` → `params["title"]`); bracket names **nest** Rack-style (`author[name]` → `params["author"]["name"]`, `tags[]` → array, `items[][sku]` → array of hashes) for form/multipart/query params alike. `f.fields_for("author") do |author| ... end` renders the bracket names and prefills from the nested document. Whitelist with `permit(params, {"title": true, "author": {"name": true}, "tags": []})` — schemaless means unfiltered mass-assignment persists anything. An unchecked `check_box` submits nothing; read it as `params["published"] == "true"`.
