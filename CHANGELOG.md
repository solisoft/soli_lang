...
* **feat(nav):** **instant navigation can morph instead of swap.** `<meta name="soli-nav" content="morph">` on a page (or `SOLI_NAV=morph` for every page, which a page can refuse with `content="swap"`) patches the live `<body>` into the new page instead of replacing it: nodes that did not change stay the same DOM nodes, so a layout's sidebar keeps its scroll position, an open `<details>` stays open and typed input survives. Elements pair by `id`, then by tag in order; unchanged scripts are not re-run, new or changed ones run in document order; Alpine components are replaced whole (state resets as with a swap) and initialized after the scripts; pages with `x-teleport` fall back to a swap. Browser specs in `tests-e2e/browser/specs/instant_nav_spec.sl`. Docs: `www/docs/views.md` → "Instant Navigation".
* **feat(ops):** **slow queries at `/__soli/slow_queries`.** Every ORM query that takes `SOLI_SLOW_QUERY_MS` (default 200) or longer — SoliDB over HTTP or the native driver, Postgres, MySQL, SQLite; in requests and in jobs — is grouped by *shape* (numbers, value lists and, in SDBQL, quoted strings taken out; in the adapters' SQL, quoted identifiers and JSON keys kept, since values are always binds) into a `_soli_slow_queries` table in the app's own database. Each shape keeps its count, average, slowest and total time, the request or job that ran it last (`GET /orders → orders#index`), 24 h of hourly counts and its five slowest runs with the query as it ran and its bind values (secret-named binds redacted, long ones cut; `SOLI_SLOW_QUERY_BINDS=off` keeps none). The page sorts by impact (total time), slowest, frequent or recent. A fast query pays one comparison — the SQL `Trace` guard now borrows the statement instead of owning it, and the SoliDB paths read the query back out of the request body only once it is slow. Same gate as the errors page (`SOLI_SLOW_QUERIES_USER`/`PASSWORD`/`TOKEN` or `SOLI_ADMIN_*`), same background writer, at most 1000 shapes per app, off under `APP_ENV=test`; `SOLI_SLOW_QUERIES=off` disables it. Linked from the dev bar's tools panel. Docs: `www/docs/observability.md` → "Slow queries".
* **feat(ops):** **notifications for errors and slow queries.** Four events — `error.new`, `error.regressed` (a resolved error failed again), `error.spike` (`SOLI_NOTIFY_SPIKE`, default 50 occurrences in 5 minutes, not for ignored groups) and `slow_query.new` — go to every destination configured: `SOLI_NOTIFY_WEBHOOKS` (Slack, Microsoft Teams — Adaptive Card —, Discord and Google Chat recognised by host; any other URL gets the event as JSON, signed with `X-Soli-Signature` when `SOLI_NOTIFY_SECRET` is set; all through the `Webhook.enqueue` SSRF guard), `SOLI_NOTIFY_EMAILS` through the app's own mailer (the dev inbox in `--dev`), and `app/jobs/soli_notification_job.sl` when the app has it, enqueued with the event hash for anything else. One message per event and group per `SOLI_NOTIFY_THROTTLE` (default 15 min); `SOLI_NOTIFY_EVENTS` narrows the set. Sent from a per-app notifier thread, so a slow webhook never delays a request. The errors and slow-queries pages say where notifications go, by product name, never the URL. Docs: `www/docs/observability.md` → "Notifications".
* **refactor(serve):** the error tracker's per-app writer (bounded queue, restart of a dead writer, panic fencing) and its database calls moved into `tenant_writer` and `internal_store`, shared now by the error tracker, the slow-query tracker and the notifier.
...
* **feat(eui):** **`tw()` takes breakpoints, spacing between children, dividers and text transforms** — still plain Soli, nothing new on the wire. `sm:` … `2xl:` resolve on the server, mobile first, against the viewport width you pass: `tw(classes, width)`, `tw_style(classes, false, width)`, or `"vw": width` beside `"tw"` on a node; a breakpoint class with no width raises instead of guessing, `tw(classes)` alone is unchanged, and the memo keys on the breakpoint, not the width. `space-x-N` on a row and `space-y-N` on a column, `gap-x-N`/`gap-y-N` along the line, become the `gap`, and the combinations that are not the same thing (a `space-y` on a row, `space-*` on a wrapping line or beside a gap, `gap-x` ≠ `gap-y` where both axes are spaced) raise saying why. `divide-y`/`divide-x` (with a width and a colour) are laid onto every child but the first by `node()`, copying each child and keeping its key and its hover styles. `uppercase`/`lowercase`/`capitalize` are applied to the string by `text()`. `mx-auto`/`my-auto` are `self: center`, `block` a column, `relative`/`static` in flow, `isolate` and `select-none` nothing, and `focus-visible:` is `focus:` with its `outline-*`/`ring-*` left to the client's own keyboard ring. A half step (`py-1.5`) is still refused and now names the two nearest steps. 550 definitions over six files.
* **feat(eui):** **the half spacing steps, and 20 and 32** — `py-1.5`, `px-2.5`, `gap-3.5`, `p-20`, `py-32`: measured as `tw()`'s most frequent refusals over ten first-draft screens (`1.5` 34 times, `2.5` 14, `32` 10, `3.5` 8, `20` twice). EUI protocol **version 6** appends them to the space scale as indices 13–17 (6, 10, 14, 80, 128 px; eui 05 §2), and `tw()` maps them there; a step the scale still lacks (`p-7`, `p-40`) raises naming the two nearest. The encoder speaks the version the session negotiated: to a client older than 6 every `DefStyle` carries the nearest older step instead, ties down (`1.5`→`1`, `2.5`→`2`, `3.5`→`3`, `20`→`16`, `32`→`24`), on the socket, on `GET /_eui/view?v=` and through `eui_render`, so no older client meets an index past its scale. `PROTOCOL_VERSION` 6 comes from the `eui-proto` rev. Docs: `www/docs/eui/styling.md` → "Tailwind classes".
* **feat(eui):** **gradients, and `pulse` and `bounce`** — a style's `bg` may be `{"gradient": {"to": "right", "stops": ["accent.base", ["#ff80b5", 255]]}}` (or `"angle": 45`; `to` a side or a corner, two or three stops, positions in 255ths, spread evenly when left out): each distinct gradient becomes one `DefGradient` per session (eui 02 §5.3), after any literal colour its stops name and before the style that names it, and malformed ones are refused with the reason. `animation` takes `"pulse"` and `"bounce"` (eui 03 §5), Tailwind's keyframes on the client's clock. All three are EUI protocol 6: a client older than that is sent the gradient's first stop as a solid `bg` and the style without the two bits, on the socket, on `GET /_eui/view?v=` and through `eui_render`. The scaffolded `tw()` takes `bg-gradient-to-*` with `from-*`/`via-*`/`to-*` and `from-10%`-style positions, and `animate-pulse`/`animate-bounce`, which combine with `animate-spin`. 556 definitions over six files. Docs: `www/docs/eui/styling.md` → "Gradients", "Arriving and leaving", "Tailwind classes".

* **feat(eui):** **a field says what goes in it while it is empty.** The scaffolded catalogue's `input`, `textarea`, `field` and every `*_field` builder take `o["placeholder"]`, sent as the `placeholder` prop of EUI 03 §3: the client draws it in `text.muted` while the value is empty, drops it at the first character, never puts it in the value or in anything the field sends, and hands it to a screen reader as the field's placeholder. The server needed nothing — a prop is a prop — so a client older than the prop simply shows an empty field. Also a pin in `src/serve/eui/tree.rs`, `a_style_only_change_is_one_set_style_per_row_keyed_or_not`: a row whose only change is its style gets exactly one `SetStyle`, keyed or not, which is what an EUI note had long claimed it did not. Docs: `www/docs/eui/widgets-input.md`.

### Changed

* **feat(eui):** **the scaffolded catalogue looks like Tailwind UI.** Labels and body text at 14 px; fields and secondary buttons white inside a `border.default` hairline (fields were a `surface.sunken` well), the primary a filled accent with a small shadow and no visible edge, every button a `control` one control (36 px) tall; field labels in the default ink (`field_label`), not muted; cards at radius 2 with a `border.subtle` ring and shadow 1, dialogs/sheets/menus/popovers/toasts at radius 3 and shadow 3, the toast a white panel with its tone in an icon; tables with a semibold header, hairline rows, 12 px of room and a local hover (`table_row(…, {"dense": true})` for a virtualised list); `rounded-md` badges and chips; tabs underlined in the accent; a sidebar and navbar whose current row is a grey wash. `accordion` accepts `children` in place of `body`. 531 definitions over six files.

* **build:** **the native SoliDB driver ships in every build.** `solidb-driver` is in the default features, so release binaries, the Docker image and `cargo install` now carry it. It used to be off because `solidb-client` was a path dependency on a sibling checkout; it is a git dependency now. Before this, `SOLI_DB_DRIVER=1` on a published binary silently stayed on HTTP, so none of the driver's read gains reached users. The runtime default is unchanged: the model layer still speaks HTTP unless `SOLI_DB_DRIVER=1` is set. CI now runs a SoliDB service (`ghcr.io/solisoft/solidb:v1.1.0`) and `tests/solidb_driver_test.rs`, which checks that rows decoded straight into Soli values equal the JSON path's and that the ORM takes that path. `SOLI_DB_DRIVER` and `SOLI_DB_DRIVER_QUERY` are now in the configuration reference
...
* **feat(eui):** **a shared element** — `shared_element(name, node)` in the scaffolded catalogue, and EUI 03 §5.3 finished on the client side so that it works. Give the same name to the node that is leaving and to the node taking its place, and the arriving one flies out of the box its partner had: a row's avatar becoming a header's avatar, a thumbnail becoming a hero. Both ends are boxes the client already laid out, so nothing is laid out again and the whole flight is one interpolation resolved once. Three things had to be true for this to be reachable at all and none of them was. The offset was between the two *corners* while the vertex stage scales about the node's own **centre**, which put the element half the difference in the two sizes away from the box it was meant to fly out of — always, since a shared element the same size on both sides is not one. Departures were read off the client's exit list, which never contains them: a release notes only the root of what it let go and a diff matches keys among siblings, so a page swap names the page and never the thumbnail inside it; the departed keys are read from the box map now, where a key whose node the tree no longer holds is a key that left, wherever it sat. And the box map is keyed by `(owner, key)` rather than the bare atom, because an island has its own atom table and atom 7 there is not atom 7 on the page. A name that resolves to nothing stays the ordinary case and not an error — a panel is built and torn down as it opens — which is also the one way to get this wrong silently, so `EUI_TRACE=1` now prints a line per pair, resolved or not, and says why. The ERP demo's customers section is one: narrow, the account's disc flies from the list row into the detail header

* **feat(eui):** **a socket that breaks is no longer an application that ended** (EUI 01 §4.1). A `Welcome` names the session with sixteen bytes; a client whose socket dropped offers them back with the sequence of the last batch it applied, and when the handle names a session still here the answer is `Welcome{Resumed}` followed by **only the batches that client missed** — no `connect`, no resync, no whole tree. The reader keeps their scroll, their focus and what they had half-typed, where before every dropped wifi hop was a fresh mount that returned them to the top. Two things in it are deliberate and neither is obvious. The handle is **minted per EUI session** rather than derived from the cookie, which is what it used to be: a digest of the session id names the *person*, and two tabs of one reader share a cookie, so a resume had nothing it could tell apart. And the handle is not sufficient on its own — 01 §4.1 makes it a bearer, so a resume also requires the socket to carry the same cookie session and name the same component, or sixteen bytes would be enough to be handed somebody else's tree with their data in it. A client that fell further behind than the last sixty-four batches is **refused** rather than half-filled, because being told you were resumed and handed a tree with a hole in it is worse than being told to start again: nothing would say so. The encoder is no longer dropped when the socket goes — that is what made every reconnect a fresh mount — and a sweep frees it when the two-minute grace runs out

* **fix(eui):** an event's **payload is checked against the shape its kind declares** before any handler runs. EUI 06 §4 asks a server to verify three things about an event — that the node exists in the tree it last sent, that it carries a handler of that kind naming that name, and that the payload has the shape §1 declares — and named `validate` as where all three happen. It did two. A `click` carrying a string, a null, one coordinate or a thousand-element list reached a view handler exactly as a pair of coordinates would, and what happened next was the application's problem. The check itself is `EventKind::payload_fits`, in `eui-proto` rather than here, because the shape of a payload is a fact about the wire format and there are seven servers — six of them separate ports of that crate. It runs first of the three, being the only one that needs nothing but the frame. An **integer is accepted where a float is asked for**, and has to be: a coordinate of exactly zero is an integer to any encoder that writes the narrowest form of a number, so refusing it would refuse the top-left corner of every node; the reverse is not true, since a fractional button, slot or row index is not a narrower spelling of anything. A failure **drops the event** rather than ending the session — §4 said otherwise and no implementation has ever done it, for the reason this server already argued in a comment: an event may arrive from a client one render behind, naming a node that existed a moment ago, and closing the session there costs a person their application for a mouse movement while making one bad frame an attack on every reader. The specification now says what the code does. `EUI_TRACE=1` prints what was dropped and why

* **feat(eui):** a view can be served as **one render, with no session behind it**. A socket costs this server a session per reader — the instance, the four interned tables, and the previous tree they diff against — which measures at 50–60 kB for somebody who is only reading, against 4 799 B of page. For a documentation page that is the wrong shape, and the socket buys nothing there, because nothing on such a page changes unless the reader changes it. Six hundred *distinct* renders now move resident memory by four kilobytes. Two roads in. `format.eui(fn() eui_render(tree))` inside `respond_to` puts the render on an ordinary route, so a page and its already-resolved form are two representations of one resource sharing a URL and its params — a browser gets the page, a client asking for `application/vnd.eui.frames` gets the render, and the match is on that exact media type and never a substring, because a browser's `Accept` ends in `*/*`. `eui_render(tree)` returns an ordinary response with an `ETag` over the body and a `304` on `If-None-Match`; its encoder is built, used and dropped inside the call, so there is no instance, no registry entry and nothing to clean up — an action is already running on a worker with an interpreter, which makes a page on a route *cheaper* to serve this way than one behind a component. `eui?` answers the same question for a controller that would rather branch itself. A component with no route of its own takes `router_eui(..., {"static": "public, max-age=60"})` and is fetched from `GET /_eui/view/<component>`, refused alongside `{"session": "required"}` because a static view is rendered for nobody. That phrase is the contract: the render sees no session, no cookie and no locale, and a `GET` has no same-origin check — a resource a CDN is meant to hold cannot have one — so declaring a view this way is promising that rendering it is a *read*
* **feat(eui):** a session can **start from the tree the client already has** (EUI 01 §2.6), and the protocol goes to version 5. A page fetched as one render opens no socket until something needs the server — and then mounted itself again, discarding the tree, the layout, focus, every scroll offset and anything half-typed, so a reader partway down a documentation page who clicked something was returned to the top with nothing focused. The client now offers the BLAKE3 of the batches it holds as a third `Hello` resume tag; Soli renders `connect` as it would have anyway, compares, and either answers with a `Welcome` and nothing else or sends the frames it just rendered. One render either way — a server that renders twice to answer this has misread it — and nothing for the application to write, since it is the same `connect` it always was. Two things are easy to get wrong and are therefore pinned by tests: the hash covers the **batches** and not the body they arrived in (a one-shot render's `Welcome` carries sixteen zero bytes and a socket's carries a session handle, so hashing the body finds no match ever and adoption silently never fires), and a refused offer must still tear the old tree down, because a fresh mount numbers from 1 again and a client that kept its acked sequence would drop it and show an empty window. Both halves ride on tag bytes that were previously refused, because neither `Hello` nor `Welcome` can grow a field — their decoders end in `finish()`, so an appended one is trailing bytes to every peer built before it. The six language SDKs need no change: they negotiate `min(client, ours)` and can only receive the offer from a client that fetched a tree over §2.4, which requires serving that endpoint
* **feat(eui):** **islands** (EUI 01 §2.7) — one corner of a page can be live while the rest stays a cached render nothing is held for. A node carrying `island`, an absolute path on the same origin, takes its *content* from a session of its own, so a documentation page with a comment count stops costing every reader a whole session. Its own children show until that session speaks, and they came from whatever rendered the page: an older client that ignores the prop, a session that cannot be opened and a page served from a cache therefore all end in the same place, content that is out of date rather than missing. Soli's half is a session address that may carry a query, arriving as `connect` params — `?for=1042` is how two islands of one component tell the application which of them is being rendered, percent-decoded, sixteen pairs, no nesting. **Nothing was added to the wire**: an island is an ordinary session addressed by an ordinary prop, which is why an application can adopt one without its clients being rebuilt. The client half — translating an island's node ids into the page's space while each island keeps its own four interned tables — is not yet built
* **feat(eui):** a browser that opens an EUI address gets **a page rather than a 404**. An application serving EUI and no HTML for a path answered browsers with an error page; it now answers with a short page saying what the address is and how to open it. A fallback and not an interception — a route the application defines always wins — and the address it prints comes from the forwarded host, since behind a proxy `Host` names an upstream nobody else can reach

* **feat(imap):** `AUTHENTICATE XOAUTH2`, via `Imap.new(host, user, "", { "xoauth2": token })`. A Google Workspace administrator can switch app passwords off for a whole domain, and by default now does — `LOGIN` then has no credential the server will accept and the account is simply unreachable over IMAP. The token is a short-lived OAuth access token minted by the caller; Soli never sees the refresh token. A refusal does not arrive as a tagged `NO`: the server sends a continuation (`+` and base64 JSON) and waits for the client to acknowledge it with an empty line before it will say anything, so a client that does not answer sits there until the socket times out. The `+` is answered here rather than left to `read_until_tagged`, which would treat it as untagged chatter and block, and a second continuation is refused rather than looped on
* **feat(imap):** `fetch_headers`, `fetch_headers_uid`, `fetch_headers_range(lo, hi)` and `fetch_headers_set(set)`. `fetch`/`fetch_uid` ask for `BODY.PEEK[]` — the whole message, every part, every attachment — which is right when you are about to read one and ruinous when you are drawing a list of twenty, where nothing but the sender, the subject and the date is ever shown. The headers family asks for `HEADER.FIELDS (SUBJECT FROM TO DATE)` instead, so a list is kilobytes, and the same parser reads what comes back: `text_body`/`html_body` are simply absent. `_range` and `_set` save the turns rather than the bytes — twenty messages fetched one at a time is twenty commands and twenty waits on a link to Google; `FETCH lo:hi` and `UID FETCH 100:*` are one of each. A sequence set is validated against the RFC 3501 grammar (digits, `,`, `:`, `*`) rather than interpolated onto the wire: an argument that could carry a space could carry a second command. Rows also carry `bytes` (`RFC822.SIZE`, which a headers-only fetch cannot otherwise know — `size` is the length of what came back) and `clips`, the attachment count read off `BODYSTRUCTURE` without downloading any of them
* **feat(imap):** `uid_mark_seen`, `uid_mark_unseen`, `uid_delete`, `uid_move`, `uid_copy`. Every mutating verb took a **sequence number**, which is a position and moves whenever anything before it is removed — so a caller holding a UID, which is what a client that stores messages actually has, had to turn it into a position first with a `SEARCH UID n`. That search is a whole round trip: ~200ms measured against Gmail, in which the application answers nothing. These are the same operations addressed the way the caller already knows how, in one turn instead of two
* **feat(mail):** parsed messages carry `parts` — every text part with the type it declares. `text_body`/`html_body` answer "the plain one" and "the HTML one", which is every message there is until one carries a third face: a `text/markdown` part is `PartType::Text` like any other `text/*`, and neither accessor will ever return it. A client that wants to *prefer* the source a message was written in has to be able to see it
* **feat(mailer):** `alternatives` on the delivered hash — an array of `{ "content_type", "body" }` — for a third face of the same message. `text_body`/`html_body` are the two mail-builder knows; a source that is neither, markdown being what this exists for, has no name in the crate, so setting a custom `body` means assembling the whole structure here, attachments included, because a custom body makes the others inert. Parts are ordered least rich to most (RFC 2046 §5.1.4: the last part a reader can show is the one it shows) — plain, the extras in the order given, then HTML — and attachments ride alongside in a `multipart/mixed` wrapping the `multipart/alternative`
* **feat(mailer):** `headers` on the delivered hash, for the headers mail-builder has no name for: `In-Reply-To` and `References`, which are what makes an answer an answer rather than a new message in the reader it lands in. This is the one place an application hands over both halves of a header line, so both are guarded — a name or a value carrying CR or LF is header injection and is refused, as is a name containing `:` or whitespace
* **feat(markdown):** `Markdown.to_text(md)`. The third face of one source: `to_html` for the readers that draw, this for the readers that do not. `strip_html` over the HTML is not the same thing — it loses a list's bullets and, worse, every link's address, leaving the words of a link and no way to reach it. Headings keep their text, list items keep a marker (`- ` / `1. `, two spaces per level), a link becomes `text <url>` unless the text already is the url, an image becomes `[alt: url]`, a quote keeps its `> `, a code fence keeps its lines verbatim, a table's cells are joined by ` | `, a rule is `---`, and emphasis loses its markers
* **feat(pdf):** `pdf_preview(template, data, options?)`, `pdf_preview_from_markdown(markdown, options?)` and `pdf_preview_response(template, data, options?)` — the same document as **PNG page images**, for a thumbnail, a preview pane or a generated image asset. The layout engine gained a second backend (`pdf/src/raster_backend.rs`) that paints the backend-neutral `LaidOutDoc` the PDF backend consumes, so the preview *is* the layout, advanced by the same font metrics, rather than a second renderer's reading of the output. Nothing new is compiled: `tiny-skia` and `resvg` were already in the graph under `svg2pdf`, whose `text` feature turns them on — naming them directly also removes the accident that our raster deps hung off someone else's feature flag. SVG artwork is drawn by resvg from an `ImageData::svg_source` retained beside the svg2pdf Form XObject, which a rasteriser cannot read. Size with `dpi` (96 default, cap 600) or `width`/`height` in px, which override it; select pages with the 1-based selection `pdf_pages` takes; `out_dir`/`prefix` write files through the same jail as `file_write_base64` and return their paths. `stationery`, `attachments`, `password`, `pdfa` and `sign` have no raster meaning: each warns and the page still renders, so one options hash drives both the PDF and its preview. Capped by `SOLI_PDF_PREVIEW_MAX_{DPI,DIMENSION_PX,PAGES,PIXELS}` because `dpi` usually arrives in a request — an A4 at 600 dpi is already a 140 MB pixmap
* **feat(pdf):** `format` (`"png"` | `"webp"` | `"jpeg"`) and `quality` on `pdf_preview*`. A page is flat colour and crisp type, the content PNG compresses worst: the `invoice` sample at 150 dpi is 165 KB as PNG and 51 KB as WebP q90, a 320px thumbnail 25 KB against 7.7 KB, with no visible ringing on body text. Encoding goes through libwebp — the same path `Image.format("webp")` takes — because the `image` crate's own WebP encoder is lossless-only and hands most of the saving back. `RasterSession::page_pixels` returns raw straight-alpha RGBA so the host encodes once rather than decoding a PNG it just wrote. The written file's extension and the response `Content-Type` follow `format`. `png` remains the default: lossless and universal is the safer default, and the docs gallery is committed as PNG
* **feat(eui):** `focus_to: true` on a node emits `Op::Focus`. `autofocus` cannot cover this — it is deliberately weak, applied by a client "only when focus is not already where it belongs" (03 §3.1) so that a batch arriving mid-Tab does not yank someone back to a dialog's first field, which makes it useless for a view that *opens* a field: a search bar opening over a page whose root holds the shortcuts never got the caret and could not be typed into. `Op::Focus` is the op the protocol has for exactly this and nothing was emitting it. Same shape as `scroll_to` — done, not held: a view that keeps asking means "stay there" and costs nothing, and one that stops asking is not a request to take focus away, because there is nowhere to put it
* **feat(eui):** `transition` accepts `"slower"` and `"slowest"` beside `none`/`fast`/`base`/`slow`, for ambient movement where `slow` is still an answer to something the viewer did
* **feat(eui):** `vu_meter` takes `labels`. Two readings are the pair a `level` event carries, so two are named left and right and one is named nothing; anything else — bands of a spectrum, a channel per voice — is the same drawing and only wants its own words. Without a name each strip announces itself to a screen reader as "Level", as many times as there are strips
* **feat(scaffold):** the root `CLAUDE.md` written by `soli new` opens with a **"Built in — check before you write it yourself"** index mapping each capability (2FA/TOTP, OAuth, uploads, PDF, jobs, rate limiting, search, push, i18n, …) to its builtin and its file under `docs/`. The docs were already bundled into every app, but nothing led an agent to them by need: asked for two-factor auth, it never found `Crypto.totp_verify` (filed under *Password Hashing* in `docs/builtins.md`) and reimplemented TOTP. A unit test (`root_claude_md_indexes_every_bundled_doc`) fails when a file added to `www/docs/` is not referenced in the index. `docs/builtins.md` gains a TOTP section with `totp_generate`, `totp_uri` and Base32 secret generation; the TOTP blog post and the Crypto docs page no longer call a non-existent `QRCode.encode`. Existing apps: `soli update docs`
* **feat(lint):** new rule **`smell/closure-cycle`** flags a closure assigned onto `this` (`this.x = fn…`, `@x = |y| …`, `this.h["k"] = fn…`). The closure captures the method environment that holds `this`, so instance and closure keep each other alive forever — a leak per object in a long-lived worker. Store a method name, or pass the closure per call.

### Security

* **security(deps):** **`cargo audit` passes with no vulnerability waived** — the first time this repository can say that. `.cargo/audit.toml` carried eleven advisories, and two of them were real vulnerabilities rather than unmaintained-crate notices: RUSTSEC-2026-0194 and -0195, quick-xml below 0.41 — a quadratic duplicate-attribute scan and an unbounded namespace-declaration allocation, both denial of service, both reachable only through `Spreadsheet.excel*` on an untrusted `.xlsx` or `.ods`. They were waived because they could not be fixed: every release of calamine and umya-spreadsheet resolved quick-xml below 0.41, and a `[patch.crates.io]` forcing it did not compile, because both call pre-0.41 APIs. calamine 0.36.1 and umya-spreadsheet 3.1.0 require quick-xml ^0.41, so the manifest moves to them and the waiver is gone. calamine needed no source change across ten major versions; umya renamed `get_sheet_mut`/`get_cell_mut` to `sheet_mut`/`cell_mut` and made the first take its index by value and answer a `Result`, which is now reported rather than unwrapped, because it runs on a request. `quick-xml` 0.31 leaves the dependency graph entirely, and so does `image` 0.24, a second copy of an already-present dependency that came in behind umya. The remaining ten waivers are unmaintained crates, each with its blocker written down
* **security(http):** **framework-reserved paths never reach the app.** A request under `/__soli/`, `/__solidev/`, `/__dev/`, `/__coverage__` or `/__livereload` (exact, `/__livereload/…`, `/__livereload_ws`) that the framework does not itself serve is a `404`. Such paths used to skip the CSRF gate *and* fall through to application routes, so a catch-all `/:slug` answered `POST /__dev` without an Origin check. `/_health`, `/_ready` and `/_metrics` are no longer CSRF-exempt — they are GET-only anyway.
* **security(dev):** **`--dev` endpoints require a local `Host`** — `localhost`, `*.localhost`, an IP literal, or a host in `SOLI_APP_HOSTS`. The dev-bar diagnostics, `/__dev/*`, the inbox, request replay, and the REPL token embedded in dev error pages were reachable by a DNS-rebinding page through the developer's own browser. A LAN name such as `mymac.local` or `myapp.test` must now be listed in `SOLI_APP_HOSTS`. The jobs dashboard is credential-free only from a loopback peer with a local `Host` (otherwise the configured Basic/Bearer credentials, `404` if none), and the inbox *clear* and *replay* POSTs must be same-origin.
* **security(http):** **request bodies are charged to the upload budget as they arrive** — a 64 KiB reservation, doubling up to `SOLI_MAX_BODY_SIZE` — instead of reserving the full cap per request up front, so small bodies no longer exhaust `SOLI_MAX_INFLIGHT_BODY_BYTES`. New `SOLI_BODY_IDLE_TIMEOUT_SECS` (default `10`): a body that stalls between two frames gets `408`. A transport error mid-body is now `400` (was `413`).
* **security(http):** **`/_metrics` without `SOLI_METRICS_TOKEN` is refused (`404`) when the request carries `X-Forwarded-For` / `X-Real-IP` / `Forwarded`, or `trust_proxy` is on** — behind a reverse proxy every peer looks local, so the loopback/private-peer allowance meant nothing there. Deployments behind a proxy must set `SOLI_METRICS_TOKEN`.
* **security(http):** HTTP/2 (h2c) connections send keep-alive pings every `SOLI_H2_KEEPALIVE_SECS` (default `30`, 20 s ack timeout) and close after `SOLI_CONN_IDLE_TIMEOUT_SECS` (default `60`) of no activity. Static files over 1 MiB are streamed from disk instead of read into memory whole.
* **security(errors):** a custom `errors/5xx` template receives a generic `message` (e.g. `"Internal Server Error"`) in production instead of the raw internal error; 4xx messages are unchanged and the real error is still logged.
* **security(session):** **cookie-driver sessions have an absolute lifetime** — `SOLI_SESSION_MAX_LIFETIME` seconds (default `2592000`, 30 days; `0` disables), counted from issue however active the session is, so a stolen cookie cannot be kept alive by use. `session_regenerate()` restarts it; legacy cookies count from their `iat`.
* **security(ssrf):** **PDF remote images go through the SSRF-guarded HTTP client** — DNS-rebinding safe, redirects followed with each hop re-validated, bodies over 20 MiB rejected. The PAdES `sign.tsa` URL is SSRF-checked too (private/loopback refused, redirects re-validated, reply capped at 1 MiB). The blocklist gains `0.0.0.0/8`, `192.0.0.0/24`, `198.18.0.0/15`, `240.0.0.0/4`, and IPv6 forms that embed a blocked IPv4 (NAT64 `64:ff9b::/96` and `64:ff9b:1::/48`, 6to4 `2002::/16`, Teredo, IPv4-compatible).
* **security(db):** document keys and collection names that are empty, `.` or `..` are refused (`invalid document key` / `invalid path segment`) in `Model.find`/`update`/`delete` and the raw SoliDB client. Field names longer than 128 characters are refused in `order`, `find_by` and hash `where` (SQL adapters included). `like` in hash filters on `includes` uses a linear-time matcher, and `_` matches one character rather than one byte.
...
* **fix(session):** a **404 from a wildcard route** no longer drops the session cookie it owes the client. The route lookup misses in two ways — nothing matched the path, or `docs#*` matched but no action answered to it — and only the first called `finalize_session_cookie`. Under the ID session drivers that was invisible, because the call returns nothing when the id has not changed. Under `SOLI_SESSION_DRIVER=cookie` it is not: the cookie driver re-emits whenever the incoming blob was invalid or expired and got replaced, so the second kind of 404 swallowed the replacement and the client went on sending the dead blob until something else happened to reset it. There is one miss now, in `serve/route_match.rs`, and it emits the cookie and writes the access-log line the wildcard miss also skipped
* **fix(dev):** hot reload picks up **`app/mailers/` and `app/policies/`**. Four directories load together into a worker's environment — models, services, policies, mailers — and that list had been written out twice, once for the initial worker load and once for the reload, where it was a directory short. So a mailer edit signalled a reload (the watcher maps it to the models signal, and says so in a comment) and the worker reloaded models, services and policies, leaving the old mailer class resident. `app/policies/` was the same defect from the other end: the worker reloaded it, but the watcher never watched it, so a policy edit on its own signalled nothing at all — it only ever appeared to work because a model edit dragged it along. The list is one function now (`app_loader::load_models_and_siblings`), called by both, and the watcher covers all four. Measured against a dev server with one edit per directory: before, models and services reloaded and policies and mailers did not; after, all four do
* **fix(check):** `soli check` **learns the runtime's global namespace from the runtime**, and stops calling working code undefined. On a 247-file application it reported **237 errors, all false**, which is how a project comes to leave it out of its verification gate — and the entry in that project's README saying so was the honest thing to write, not the decision to fix. Five families, and the first was the whole hand-written-list problem: 140 of the 237 named something the interpreter installs — `describe` and `res_body` from the test DSL, `RateLimiter`, `middleware` from the routing DSL, `Mailer`, `form_with` — and the checker's own list of engine classes was a second list beside `register_builtins`, drifting from it by construction. It is derived now: the builtins are registered into a throwaway environment which is then asked what it holds, the six preludes the runtime evaluates as **Soli source** (routing, mailer, form builder, uploads, retry) are parsed for the functions and classes they declare, and a class global carries its native verbs — so `I18n.cache_table`, real since the day it was added to the interpreter and missing from the checker's hand-modelled surface, resolves, while `I18n.tarnslate` is still an error. A name that is already modelled precisely keeps its type; nothing here overwrites one. Two sets are seeded **per file** instead, because they mean nothing everywhere: the test DSL for a file `soli test` would run, and the request scope — `req`, `params`, `session`, `cookies`, `current_user`, plus `render` and `redirect` — for a file under `app/`, `config/` or `stdlib/`. So a `describe(…)` in a controller and a `render(…)` in a standalone script both still fail, the second being a decision this codebase had already written down and pinned with a test. Computed once per process behind a `OnceLock`: the pass over those 247 files takes 0.4 s. The parity ratchet that exists for exactly this class of bug records the consequence — its `[type-checker]` section drops from **312 names to 62**, and a new test holds what is left to one of two stated reasons (test DSL, request scope) so the section cannot drift back into a bucket where a missed builtin is silenced by adding a line. Then four narrower ones, each a case where the checker's answer was not what the language does. A method with no return annotation was typed `Void`, so every use of what it returns was an error (`poses + Job.pour_organisation(...)` read as "cannot add Int and Void") — annotations are optional in Soli and `check_class_stmt` already typed those bodies as `Any`; only the declaration tables disagreed, and now they do not. A class whose parent this pass never saw declared looked like a root with only its own members, which is every controller in a served application (`ApplicationController < Controller`) and every class whose parent sits in a sibling file (`UserPolicy < ApplicationPolicy`), so member access on one answers `Any` — as it already did for `Model` subclasses — and so does a `module`, whose `this` is whatever includes it. `let retenue = nil` was typed `Null` rather than "nothing known yet", so the next line's `retenue["applies_on"]` was "cannot index Null" on code that runs. And writing a key into a hash was checked against the value type of the literal that built it, so `{"timeout": 10}` followed by `e["Authorization"] = "Bearer …"` was "expected Int, found String" — a Soli hash is a heterogeneous bag, and a snapshot of one is not a declaration. What still fails, verified case by case: a typo'd variable, a typo'd verb on a builtin class, an `Int` annotation given a `String`, three arguments to a two-parameter function, an unknown member on a class whose whole ancestry is in the file, and indexing the result of a function annotated `-> Void`

* **fix(interpreter):** **a throwaway interpreter no longer keeps its whole builtins registry alive.** `register_retry_class` evaluated `retry.sl` into the environment it was registering into, so `Retry`'s methods closed over that environment while the environment owned `Retry` — a reference cycle neither side ever freed. An `Interpreter` is built for every named-scope call, every user method on a primitive, every validator, so a server kept a registry (~350 KB) per call and a page making a few dozen scope calls grew its worker by megabytes on every request. The class is now evaluated once per thread into an environment of its own and defined into each interpreter by reference; two tests pin that dropping an `Interpreter` (plain or `new_for_serve`) frees its globals

* **fix(dev):** **the flamegraph panel no longer freezes the page it is profiling.** It drew a chart rect and a list row for every span, and a request walking a few hundred records produces thousands: 3.4 MB of markup on a real dashboard, laid out on every page load whether or not the panel was opened. It now draws the **300 heaviest spans** — by duration, not by order, so a cut never hides the expensive ones — and its header says how many it left out; `SOLI_DEV_FLAME_MAX` changes the bound and `0` removes it. `trace.json` stays complete: above 64 KB it is no longer inlined as a `data:` URI but served from a new dev-only `/__solidev/trace/<request-id>` endpoint

* **fix(eui):** an EUI or LiveView **event now begins by emptying the per-request logs**, the way an HTTP request does. Those logs — queries, HTTP and KV calls, phases, middleware, views, spans, routes, template warnings — are thread-local `Vec`s a worker fills as it serves and a dev bar reads at the end, and the only place they were ever emptied was the front door of an HTTP request. A socket event never went through that door, so on a realtime worker they only ever filled. In `--dev` the flamegraph records a span per **function call**: a view that redraws ten times a second grew that log by close to a megabyte a second, for hours, until the window went slow and then froze — and it read like a leak in the application, because nothing the application held was growing. Measured on herdr-eui, the same page, the same daemon: 0.85 MB/s with `--dev` and 0.16 MB/s without, before; 66 → 72 MB over a minute and then flat, after. The block that empties them is one function now, `forget_request_logs`, called from both doors; it is a `Vec::clear` on an empty vector when nothing is recording, and it also runs for production detail logging and OpenTelemetry, which reuse the same buffers

* **fix(db):** a failed SoliDB connection says *what* it failed to connect to. The message was `Failed to connect: <io error>` at each of the twenty-four `Solidb` verbs and never named the host, because the four-line connect preamble was written out at every one of them — so there was nowhere to make that change that was not twenty-four changes. Those four lines are one function now, and the host is in the message
* **fix(lang):** `match` no longer carries three pattern forms nothing can write. `MatchPattern` declared ten variants and the parser produced eight: `1 | 2 => …` is a parse error, and so is `Point { x, y } => …`, whose branch sat behind a guard requiring the identifier to be followed by `:` — so the `{` it then looked for could never be there, and its `fields` vector was always empty. All three were handled and maintained in the parser, the tree-walker, the type checker, the formatter and the VM's compilation gate, which is how a feature that does not exist comes to look like one that does. Deleted rather than given a syntax: giving `|` a meaning is a language change and belongs in a proposal. The reachable spelling `v: Type` is untouched, and the bytecode VM now compiles **every** pattern the parser can build

* **fix(db):** `with_transaction` and every `transaction { … }` block against a current SoliDB. `begin_transaction` read the new transaction's id out of a `tx_id` field; the server answers `{"id": "tx:…"}`, so the id was never found and every block died at the first statement with `failed to begin: No tx_id in response`. Both implementations (`model::crud` and `solidb_http`) now accept either spelling, and the error carries the response body instead of naming a field that may not be the one missing
* **fix(db):** `db_query_hardcoded()` no longer returns an empty string when the server refuses the query. It discarded the HTTP status entirely, so a 401 — which SoliDB answers with an empty body — was indistinguishable from an empty result set. It now reports the failure through the same `format_query_http_failure` its sibling `db_query_raw` has always used, honouring the documented contract that the result is never empty
* **fix(types):** `Image`, `File` and `Trusted` are known to the type checker. All three are installed unconditionally by `register_builtins`, but none was in the engine-embedded class list, so any script touching them failed the check with `Undefined variable 'Image'` unless it was run with `--no-type-check` — the exact problem that list was added to solve, with three classes missed
* **fix(lint):** ten `pdf_*` builtins are in the linter's known-globals list. `pdf_render`, `pdf_response` and the two Factur-X entry points were there; `pdf_from_markdown`, `pdf_layout_map`, `pdf_fill`, `pdf_merge`, `pdf_pages`, `pdf_stamp`, `pdf_sign`, `pdf_verify`, `pdf_extract_facturx` and `pdf_attachments` were not, so `smell/undefined-local` flagged every call to them
* **fix(types):** `pdf_response` has a type signature. The PDF block listed thirteen builtins and omitted this one
* **fix(types, lint):** **the EUI builtins are known to `soli check` and `soli lint`, and a test now stops the next family from being missed.** `router_eui`, `eui_render`, `eui?`, `eui_wake`, `eui_notify`, `eui_asset`, `eui_font`, `eui_icon`, `eui_name`, `eui_stats` and `eui_capabilities` were registered at runtime and declared to neither surface, so `soli check` answered `Undefined variable 'router_eui'` and `smell/undefined-local` flagged every call — on an application built by the framework's own `soli new --eui`. `sse` and `stream` were in the same state; `next` was known to the linter and not the checker, which is why it only ever worked inside a server handler; `permit` was the reverse. The lint list was also missing fifty-odd ordinary builtins a controller calls without thinking — `cache`, `forbidden`, `json_stringify`, `password_hash`, `md5`, `sha512`, `url_encode`, `slurp`, `broadcast`, `csrf_token`, the `x25519` family — each one flagged at every call site. This is the third time this cycle the same defect has been fixed by hand (ten `pdf_*` builtins, then `Image`/`File`/`Trusted`), and each time a user found it, so it is now pinned: `tests/builtin_registration_parity_test.rs` enumerates the runtime environment — the same authoritative source `examples/builtin_inventory` uses, because grepping the registration sites misses whatever is registered in a loop — and holds every global against both lists. It is a ratchet against `tests/builtin_registration_baseline.txt`, in the spirit of `scripts/unwrap_baseline.txt`: a name that is not in the baseline fails the build, and a name in the baseline that has since been registered fails it too, so the file can only shrink. The baseline is not a list of bugs — most of it is DSL that is only legal inside a spec, a model body or a migration, and `render`/`redirect` are in it as a decision rather than a backlog item, with the reason written next to them
* **fix(int/float):** `Int.to_i` and `Float.to_f`. Every type answered `to_i` — a string parses, a float truncates, `null` is 0, a bool is 0 or 1 — except the one type that already *is* an int, which raised `Cannot access property 'to_i' on int`. `Float.to_f` was the same gap mirrored. That is precisely backwards: the reason to reach for `.to_i` is that you do not know what arrived, and the only value it refused was the one already in the right shape, so every call site had to write `(x ?? 0).to_i` to defend against the case that needed no conversion at all. Both conversions are now total over the scalars, and `params["page"].to_i` can be written once. The template engine already had both identities, so `<%= n.to_i %>` in a view worked where the same expression in a controller did not

* **fix(datetime):** `DateTime.now()` carries the subsecond that `millisecond()` reads. It was `timestamp() * 1_000_000_000`, so every instant it ever made had a zero subsecond: `millisecond()` answered `0` for all of them, and a duration measured between two `now()` calls could only come out as a whole number of seconds — which is not a measurement. Anything timing itself in Soli was timing nothing. `utc()` beside it has always kept them
* **fix(image):** `Image.new` reads the format from the bytes, not from the name. `ImageReader::open` infers it from the extension alone, so a file named by content hash — which is what a cache of downloaded images looks like — failed to decode however ordinary a PNG it was. Sniffing is what every other reader of these formats does, the magic bytes are unambiguous, and the extension is not ours to control when the file came off the network
* **fix(mailer):** a named recipient is one address, not two. `to` and `cc` were handed to the builder as bare strings, so `"Ana <ana@x.io>"` was written as `To: <Ana <ana@x.io>>` — one address inside another, which is not an address at all. They now go through the same parse `from` has always had
* **fix(eui):** a vertical VU meter is the mirror of the horizontal one. A segment took no growth vertically and a 3px minimum, so the bar came out the height of its contents — 58px — while the legend beside it stretched to 96, and no mark stood against the segment it names. Both axes now share the length the strip is given; the scale uses `justify: "between"` over that same height, and takes no `height: "100%"` — the parent row is already `align: "stretch"`, so asking for a percentage on top of it resolved against an ancestor and laid the meter out 6232 pixels tall
* **fix(coverage):** a file linked by a symlink is no longer counted twice. An application that shares code by symlink saw every shared file listed under two keys — once at the path the application reads, once at the path `canonicalize` resolves it to — one of them at 0.0%. The source walk records executable lines under the application's path while the test runner's preamble canonicalized, so functions declared there carried the resolved path and their hits fed a phantom entry. `coverage_path_key` gives one key: absolute, so the walk (relative) and the loader (absolute) meet, and without `canonicalize`, so a link stays the file the application believes it is reading
* **fix(coverage):** a body run by a fresh interpreter keeps its file. Seven places build an `Interpreter::default()` to run a user function's body — model scopes, receiver-bound methods, custom validators, validation conditions — and that interpreter is new: empty call stack, `current_source_path` at `None`, so none of those lines were attributed to any file. A `scope(...)` called hundreds of times stayed at zero hits in every model in every project, and the small file that declares it looked far less covered than it is. Each site now carries `func.source_path` — the file that *declares* the function — onto the new interpreter
* **fix(coverage):** a line is attributed to the file that carries it. `execute_statement` charged each executed line to `current_source_path` — the *entry* script: the spec, or the request. Every hit from a called function therefore landed on the **caller's** file, and the file that holds the code came out empty: a controller whose three routes are tested and whose response bodies are asserted line by line reported 2.7%, with lines counted uncovered that run on every request. The repository already knew the trap and says so a few hundred lines below, for error locations, and `record_coverage` already preferred `current_file_path()` for the same reason; only the statement path was left on the old form. On a real application (187 spec files, full browser suite) 66 files change: files that would not move despite tests that plainly exercise them go up, and three controllers drop below 90% because they had been living off their neighbours' hits
* **fix(test):** the file name comes before the bar in the worker dashboard. A worker row laid down a 14-cell bar whatever the width, then gave the name what was left — its 4-character floor everywhere under 37 columns, so a narrow pane showed `dos…` and the row ran past the terminal without saying so. The row is now composed by priority: label, spinner and name are always there; the counter, the elapsed time and then the bar are added only if they fit, and are dropped rather than take the name below fourteen characters. The bar goes first — the one at the bottom already says how the run as a whole is going — and spends only the surplus above twenty-eight characters of name, so widening the terminal never shortens a name. Two corollaries: the aggregate bar sizes itself too (fixed at 30 cells it wrapped its line under 70 columns, and a wrapped line breaks the `\x1b[{n}F` rewind, so the dashboard stacked on every repaint instead of repainting in place), and `COLUMNS` now outranks the ioctl, whose 0 is discarded, so a pane whose width cannot be asked for can declare it
* **fix(session):** **the in-memory session store really frees expired sessions** — swept every 1000 creations or 30 s — and is capped by `SOLI_SESSION_MAX_IN_MEMORY` (default `100000`, `0` = unlimited); past the cap the least-recently-used sessions are evicted, which logs those users out. It used to grow for the life of the process.
* **fix(dev):** the query/HTTP/KV logs keep at most 10 000 entries per request, WebSocket event or background job, and are reset per WebSocket event and per job instead of accumulating across them.
...
* **perf(vm):** zero-argument controller actions — the scaffold's `def index` — run on the bytecode VM; they were always sent to the tree-walker.
* **perf(http):** HTML responses are no longer copied when no script is injected, and the ETag hash is faster. ETag values change once after upgrading, so clients revalidate once.
* **perf(db):** the SoliDB JWT no longer takes a global write lock per query, and a token refresh no longer blocks other workers. The DB registry is no longer deep-cloned per query; SQL adapters remember the tables they created instead of running `CREATE TABLE IF NOT EXISTS` before every write; SQLite gets a statement cache.
* **perf(session):** the `solidb` and `solikv` session drivers load the session document once per request.
* **perf(static):** the public directory is canonicalised once, and the asset cache is consulted before the filesystem.
...
* **feat(lint):** new rule `idiom/prefer-to-s` flags `x ?? ""`. Coalescing to an empty string means "render this, and render nothing when it is nil", which is what `.to_s` says in one call. It is also the more honest version whenever the value is not already a string: `count ?? ""` evaluates to the *number* when there is one and to `""` when there is not, so what the expression yields changes type with the data, while `count.to_s` has one type always. A real fallback — `name ?? "Guest"` — is left alone, since it says something `.to_s` cannot
...
* **fix(scaffold):** `soli new <app> --eui` no longer writes a slider that can end the session. The catalogue it copies gated the track's `pointer_move` handler by *declaring* it only while a drag was live — the shape spec 03 §3.4 rejects, because an event already in flight then names a handler the server has since removed, 06 §4 refuses it and the session goes with it. Measured on a split pane, that was 167 of 429 events dropped. `slider` and `range_slider` now declare `track` (03 §3.4) — `track_min`, `track_max`, `track_step` and `track_value` on the node, `track_part` naming the groove, the fill and the thumbs — and one `change` handler that never leaves the tree. The client resolves the whole gesture and reports the value only when the quantised value has moved, so a drag costs one event per step crossed rather than one round trip per pointer sample; the builders keep no arithmetic and no `width` to invert an offset against, since the only width that was ever true is the one the client laid out. **A new application generated by this release needs an EUI client that understands `track`** — an older one lays the groove and fill out at zero width and has no handler to grab

### Other

* **chore(scaffold):** the catalogue caught up with eui's light-dismissable dropdown — `dropdown` takes an `on_close`, and a press outside the widget reaches the overlay as `blur` without the server being asked
* **feat(eui):** the manifest says which component a bare origin opens
...
* **fix(template):** the test-only builtins no longer resolve inside a view. The template engine built its environment with `register_builtins(env, true)` unconditionally, so `visit`, `click`, `assert_eq`, the factories, the expectation class and the mock-HTTP helpers were defined inside every rendered `.html.slv` in serve mode — names `register_builtins` refuses everywhere else in serve mode precisely because leaking them into a served app is a hazard. They are gone from views in every mode; `soli test` is unaffected, because `Interpreter::new` still registers them into the environment it owns
* **perf(interpreter):** a worker thread builds the builtins once, not three times. `register_builtins` defines ~500 bindings, ~100 of which are `Value::Class` values, and none of it depends on the application — yet each worker built the whole registry three times over and kept all three alive: `Interpreter::new_for_serve` for the globals, `core_eval::get_builtins_rc` for the template engine, and `load_view_helpers` for the helpers' closure. A thread-local `SERVE_BUILTINS` now holds one registry per thread and the three consumers are child `Environment`s enclosing it, so an application binding still shadows a builtin exactly as before and nothing is shared across threads (the values stay `Rc`). **On RSS this measures as nothing.** Five alternating A/B pairs at `SOLI_WORKERS=16` on `www/` put the difference at +0.4 MiB with a per-sample spread of ~12 MiB — a registry is a few hundred KB, not the megabytes first assumed, so two fewer of them per thread stays under the noise floor. It is kept for the redundant boot work it removes and because it is the shape the multi-tenant work needs, not for a memory win
* **refactor(serve):** per-application state has a seam to move behind. `src/serve/tenant.rs` holds a registry of `Tenant`s, one per served application, and each thread knows which it is serving; worker threads are pinned, so the current tenant is fixed when the thread starts rather than looked up per request. With one application the registry holds exactly one tenant (`TenantId::PRIMARY`) and every thread falls back to it, so nothing observable changes. Moved so far: the app root (was `live::component::APP_ROOT`, a `lazy_static` `Mutex<PathBuf>`) and the `File` / `Image` jails (were `OnceLock`s). The jails are the ones that matter — one shared jail in a process serving two applications would let either resolve paths under the other's root — and they keep their first-write-wins semantics, because swapping a jail mid-run would leave in-flight requests resolving against the old root. The remaining ~25 process-global singletons are listed in `www/docs/internals/serve.md`

* **refactor(serve):** every remaining per-application singleton but three is now keyed by tenant. `TenantCell<T>` (for `Mutex<Option<T>>` / `OnceLock<T>`), `TenantValue<T>` (for `lazy_static! { RwLock<T> }`, with the constructor in the declaration so it replaces `lazy_static!` and stays `const`) and `TenantLocal<T>` (for `Rc`-bearing thread-locals) let a singleton keep its shape and change only what it is. Converted: `VIEWS_DIR`, `PUBLIC_DIR` and `TEMPLATE_CACHE` — `init_templates` was first-caller-wins, so a second application in the process would have rendered the first one's views; `JAR_CACHE`, which holds the keys that sign and encrypt cookies, derived from that application's `SOLI_SESSION_SECRET` (shared, either application could mint a cookie the other trusts); `MOUNTED_ENGINES`, since which engines are mounted and at which prefix belongs to one app's config; and `MAILER_CONFIG`, which holds one app's SMTP credentials and `from` address (shared, a co-hosted app would send through them, and `Mailer.configure` in one would reconfigure the other). `TRUSTED_PROXIES`, one app's deployment topology — a co-hosted app behind a different proxy, or behind none, would inherit the list and honour `X-Forwarded-*` from a client that reached it directly; and `RATE_LIMIT_STORE`, whose buckets are keyed by whatever the app throttles on, so sharing would let one application's traffic spend another's budget. `SOLIKV_CONFIG` with the `RESP_POOL` beside it, since a shared pool would hold sockets to whichever host was configured first and `Cache.clear()` in one app would reach into the other's store; and `CONTROLLER_REGISTRY`, which otherwise answers another app's layout lookups and before/after-action resolution. and `SECURITY_HEADERS_CONFIG` with `SECURITY_HEADERS_ENABLED` — a CSP, an HSTS policy and a frame policy belong to one app's threat model, so shared, `set_csp(...)` in one would rewrite the other's policy and `disable_security_headers()` in one would strip the baseline from both. and `MODEL_REGISTRY` with the four `COLLECTION_*` maps beside it — two applications routinely declare a model of the same name, and a shared registry hands one the other's validations, callbacks, relations, encrypted fields and connection routing. Three are deliberately left process-global, each for a reason recorded in `www/docs/internals/serve.md`: the mixin hooks (keying them per tenant loses every `included do` on the second app to compile an identical module, because `MODULE_CACHE` is keyed by source text), the `ROUTES` thread-local (already per-application under thread pinning, and keying it costs a hash lookup per request), and `.env` (process-wide whatever the registry does)

* **refactor(serve):** mounting an application can happen from any thread. Mount-time state lands on whichever tenant the calling thread is bound to, so a host mounting a second application from the thread that booted the first would overwrite it. `tenant::scoped(id, || …)` binds for the duration and restores the previous binding afterwards — on panic as well, so a mount that fails halfway cannot leave the thread pointing at a half-built tenant. `tests/tenant_isolation_test.rs` mounts two applications in one process and asserts each sees only its own root, jail and collections, that a thread pinned to a tenant reads what the mounting thread wrote, and that the primary tenant is untouched by either — the last being what makes the whole conversion a no-op for `soli serve`

* **refactor(serve):** the request path takes one per-application bundle instead of seven arguments. `handle_hyper_request` received the worker queue, the reload channel, the public dir, the asset cache, both realtime senders and the dev flag separately; they are now a `TenantRuntime`, destructured at the top so the 1400-line body is untouched. That is the shape a multi-application host needs — choosing who serves a request becomes one lookup against one value from the `Host` header rather than seven parallel maps — and with a single application there is exactly one of them and nothing on the request path changes

* **feat(serve):** requests are routed to an application by `Host`. `src/serve/vhost.rs` holds a `Router` that a single-application server builds with `Router::single(...)` — one fallback entry answering every host, and no host at all, so `soli serve` behaves exactly as before. A host serving several builds it with one `insert` per mounted application, and a request whose host nobody claims gets **421 Misdirected Request** rather than whichever application happens to be first. The header parsing is tested on its own, because it has traps: `Host` is case-insensitive, a port is not part of the site, `example.com.` is the same name as `example.com`, and an IPv6 literal has colons inside its brackets — the naive `split(':').next()` turns `[::1]:8080` into `[`. A host claimed twice is reported rather than silently reassigned, since which application lost would otherwise depend on mount order. HTTP/2 sends `:authority` instead of a header, so the lookup falls back to the URI authority

* **fix(db):** the database layer is per application. `db::registry` (which connections exist and which is default), `db::config` (the default adapter and URL) and `CACHED_DB_CONFIG` (SoliDB cursor URL, database name, API key, basic auth) were process-global `OnceLock`s, and `DB_CONFIG` a process-global `lazy_static`. In a process serving two applications the *first* to boot would have frozen all of them for every application after it — not with an error, silently: each later application reading and writing the first one's data, signed with the first one's credentials. This is the blocking prerequisite for hosting more than one application in a process, and the most consequential of the singletons converted in this cycle. The JWT state (`JWT_CACHE`, the login backoff and the consumed-`SOLIDB_JWT` flag) becomes one per-application struct rather than three globals, because its parts move together — a token belongs to one application's credentials, and a backoff recorded after *its* login failed must not make another application skip its own. `get_api_key` and `get_basic_auth` now return `String` rather than `&'static str`, and `db::config()` an owned `AdapterConfig`: a per-application value cannot be lent out of the lock it lives in. `db_host()`, `db_scheme_and_host()` and `with_soli_db_config()` replace the `DB_CONFIG.host` / `DB_CONFIG.scheme` derefs

* **fix(serve):** the realtime, policy and upload state is per application. A second sweep after the database layer, over what a *request* reaches rather than what boot writes: `CORS_RULES` (one app's allowed origins would apply to another's `/api/*`), `CSRF_SKIP_PATTERNS` (`skip_csrf("/webhooks/*")` in one app would disable the barrier on another's), the LiveView upload store and its partial-chunk map (user bytes keyed by a server-minted id), `BG_SENDER` (a pool thread builds its interpreter from *its* app's models and talks to *its* database, so a second app's jobs would have run against the first one's everything), `LV_EVENT_TX` / `PINNED_LV_TX` (an app's EUI sessions rendered by workers that never loaded its code), the LiveView and WebSocket registries (live instances keyed by session id with their per-instance frame locks; live sockets and the rooms they joined, so two apps both using a room called `"lobby"` would broadcast into each other's), the live-query subscriptions, the LiveView routes, tick tasks and room declarations, the EUI manifest capabilities and signature cache (what one app asks the viewer to grant it), and the EUI stats and dev-bar snapshot stores. The last three were `OnceLock`s, so the failure was not a race but deterministic and silent: whichever application booted second got the first one's. `put_chunk` keeps its explicit lock release — it assembles a finished file, releases the chunk lock and only then calls `put`, which takes the store lock; wrapping the body in a closure would have nested the two and fixed a lock order the file deliberately does not have

* **fix(session):** the session config and store are per application. `SESSION_CONFIG` carries the TTL, the cookie attributes and — through `SOLI_SESSION_SECRET` — the key that seals a cookie-driver session, so two applications sharing it would share the sealing key and either could mint a session the other trusts. `CURRENT_STORE` is looked up by session id, so a shared store would resolve one application's id inside another. Both are now `TenantValue`s; `configure_session` builds the store *before* taking either, since `create_store` can reach SoliDB or SoliKV and neither value should be held across that

* **fix(serve):** routing, schema and signing state is per application. A third sweep: the EUI component→view map, the per-instance encoders and the `session: required` policy (two apps may both register a component called `counter`, and this decides whose view renders it and whether an upgrade is refused); the WebSocket route table (`/chat` in one app is not `/chat` in another); the introspected table-shape cache, keyed by (connection, table) — `primary` and `users` are exactly the names that collide, and column mode builds its SQL from that shape; and the EUI **publisher key**, which is a private key read from the application's own `config/eui_publisher.pkcs8` and whose public half clients pin — a process-wide key would have signed every co-hosted application's manifest with whichever booted first, so a client that pinned one publisher would silently accept manifests from another

* **refactor(serve):** review follow-ups on the tenant seam. The per-thread shared builtins root from earlier in this cycle is **reverted** — it measured as no RSS win and changed behaviour in two ways separate registries never did (a bare `logger = …` inside a function became a short-lived local; `Model.define_method` mutated a class that outlived the interpreter and survived hot reload); the fix that views no longer see test-only builtins stays. `TenantValue` now holds **one lock per tenant** instead of one over all of them, so a closure that blocks under `write` — the database JWT login, for up to its network timeout — stalls that tenant alone, and first-use construction runs outside every lock. `scoped()` sets the task-local as well as the thread-local, so a mount from inside a request handler lands on the tenant it names. Worker, background-job, job-poller and webhook threads now actually `bind_current` to the tenant that spawned them, which the docs had claimed and nothing did; every spawn on the request path goes through `tenant::spawn`, and the LiveView reaper is one per application rather than a process-wide `OnceLock` that starved every tenant but the first. `TRUST_PROXY_ENABLED` and the i18n store are per application — the former was the lenient half of a per-tenant check, so one app's `trust_proxy(true)` made a co-hosted app honour attacker-supplied `X-Forwarded-*`. The `Router` no longer allocates two strings per request to probe a map a single-application server leaves empty. The docs-site changelog gets back the `<% unless %>` entry this branch had deleted from `main`'s Unreleased section

* **fix(serve):** three defects found by review of the tenant work. (1) `TenantValue::read` treated a poisoned lock as "no value" and retried, so a closure that panicked under `write` sent every later reader into an infinite spin; every lock in `tenant.rs` is now poison-tolerant, since a jail reading as "none" or an app root falling back to `.` after an unrelated panic are wrong answers that look right. (2) The shared builtins root was writable from below: `print = 42` in application code walked the environment chain and rewrote the registry the template engine and view helpers resolve through, and the change survived hot reload because the root outlives the interpreter. The root is now sealed — `assign` answers `NotFound` so the caller shadows in its own scope, which is what a separate registry per consumer used to give it; verified on a served app where a controller hijacks `str` and the view still renders the real one. (3) The HTTP request future is scoped to its tenant with a tokio task-local (`tenant::task_scope`), consulted before the thread-local, because a request moves between tokio threads at every `.await` and the CORS/CSRF/cookie/session state it reads is keyed by tenant; `cors::evaluate` moves inside the scope, after the host is resolved. Spawned realtime tasks still start unbound and are listed as step-4 work rather than assumed

* **build(scripts):** `scripts/mem-probe.sh` measures what a `soli serve` process costs, across `SOLI_WORKERS` values. `ps -o rss=` cannot separate a process's own heap from the file-backed pages every `soli` process shares, so it reads `Pss_Anon` + `SwapPss` from `/proc/<pid>/smaps_rollup` — both halves, because `Pss_Anon` counts residency and a swapping machine makes a process read as *smaller* the more pressure it is under. Every sample runs with `MIMALLOC_PURGE_DELAY=0` (the default delay leaves the one-time boot-parse churn mapped) and each point is repeated, reported as a median with its range: transparent huge pages get collapsed and split on khugepaged's schedule, which moved anonymous RSS by tens of MiB between otherwise identical runs. `--ab OLD NEW <app> [workers]` alternates two binaries within one session, which is the only comparison worth trusting — two sweeps taken minutes apart also measure whatever the machine did in between, and that drift lands entirely on whichever ran later

### Fixed

* **fix(test):** the whole test binary no longer aborts on two recursion tests. `unbounded_recursion_errors_instead_of_overflowing` and `unbounded_constructor_recursion_errors_instead_of_overflowing` drive the interpreter to `MAX_CALL_DEPTH` on libtest's own 2 MiB thread. With `debug_assertions` off the cap is 256 frames, and `[profile.ci]` sets `lto = false`, so frames are larger than in the LTO'd release build the number was measured against — the recursion reached the end of the stack first. A stack overflow aborts without unwinding, so it took the test process with it and every test ordered after those two went unreported (`signal: 6, SIGABRT`). Nothing in Soli runs interpreted code on a 2 MiB stack — `soli run` uses the 8 MiB main thread and every serve/job worker gets `worker_stack_bytes()`, 64 MiB — and the runtime is correct in those contexts: the release binary answers unbounded recursion with `call stack too deep (256 frames)` and exits 70, on the tree-walker and the VM alike. Both tests now run on a worker-sized thread, so they test the cap rather than libtest's default
* **fix(test):** `db::tests::solidb_default_is_ready` no longer fails wherever SoliDB is not on `localhost:6745`. Its fixture hard-coded that host and `default` as the database — the *fallbacks* `ensure_runtime_ready` uses when `SOLIDB_HOST`/`SOLIDB_DATABASE` are unset, not the values it compares against when they are set — so a machine pointing SoliDB anywhere else (CI's disposable instance answers on another port) read the connection as a per-connection host mismatch. The same fixture backs `solidb_connection_pointing_elsewhere_errors`, which was therefore passing for the wrong reason: its *primary* connection was the mismatch, not the `analytics` one under test. The fixture now reads the same env with the same defaults
...
* **fix(test):** the dev-bar browser spec has a dev-mode server again. Test servers moved to production mode so specs exercise the bytecode VM, but the dev bar only exists under `--dev`, so `dev_bar_spec.sl` was left asserting against a page that no longer had one — six failing assertions that failed the `browser` job. It now lives in `tests-e2e/browser/dev-specs/`, which CI runs in its own invocation with `SOLI_TEST_SERVER_DEV=1`; every other spec keeps its VM coverage
...
* **feat(eui):** chart colour roles. `series.1`–`series.5` join the theme's role table (33 roles now), matching `eui-theme`'s `Role::Series*`, so a chart can name a categorical series instead of hard-coding a hex. A view using one was previously refused with `EUI: unknown colour role 'series.1'`
* **feat(eui):** `scroll_to` on a node, emitted as a `ScrollTo` op by the differ and the bare-tree walk, so a server-driven render can move a scroll position rather than only describe one
* **feat(eui):** `position: "pointer"` (`Position::Pointer`), for a node placed where the pointer is rather than in the flow or a stack
...
* **fix(test):** a suite runs on the engine that serves production. `soli test` started its servers with `--dev`, and `serve` only builds a bytecode VM when `!dev_mode` — so every spec ever written ran on the tree-walking interpreter while production ran on the VM. A handler the VM refuses passed the whole suite, and `SOLI_FAIL_ON_VM_DEMOTION` could not catch it, there being no VM to demote from. Nothing was gained by `--dev` here: hot reload watches files nobody edits in a process that lives one run, and the per-request dev instrumentation is work no spec asked for. Measured at equal build profile on a 142-spec suite: 114.9s before, ~114s after — the same, and now with zero VM demotions across the run
* **fix(test):** the worker databases an interrupted run leaves are swept at the start of the next one. The post-suite drop only knows the databases of the run it is ending, so a `Ctrl-C`, a timeout or a panic left them behind, as did the extra workers of a run made with a wider `--jobs`. One SoliDB instance had reached 117 abandoned databases holding every SST file open. Only `{stem}_w{N}` names built from the run's own base are candidates — never the base itself, never a name another application could own
* **fix(test):** a killed runner takes its test servers with it. `ChildGuard`'s `Drop` covers every way a suite can finish and none of the ways it can be killed, so a `Ctrl-C` that escalated left `soli serve` children reparented to init — three were found alive six hours later, still holding SoliDB connections and re-creating their `_test` databases the moment anything dropped them. `PR_SET_PDEATHSIG` now asks the kernel for the guarantee, as `cdp::arm_parent_death` already did for a browser
...
* **build:** `eui` joins the **default** feature set (and `full`, so it stays a superset). v2.1.0 shipped EUI behind `--features eui`; a binary installed the ordinary way (`cargo install --path . --locked`) therefore had no `router_eui`, and any app whose `routes.sl` registers an EUI component failed to boot with `Undefined variable 'router_eui'`. `eui-desktop` — the winit/wgpu window `soli desktop build --eui` needs — stays off. Drop EUI again with `--no-default-features` plus the features you want.

### Security

* **security(eui):** an EUI component was reachable over the JSON LiveView socket. `router_eui` registers the handler in the registry `/live/socket/<component>` reads, and the worker dispatches on the component name, so a JSON event could reach an EUI handler with an event name and a `params`/`props` hash of the client's own writing — the check that every binary-socket event is validated against the tree the client was last sent never ran. `/live/socket/<eui component>` now answers 404
* **security(eui):** `connect` can refuse a client: a handler may return `{"close": reason}` (or `close: true`) from `connect` or any later event; the client gets an `Error` frame (403) with the reason, nothing is rendered, and the session ends. `router_eui(component, handler, view, {"session": "required"})` refuses an upgrade with no session cookie with 401 before any handler runs. No middleware runs for a WebSocket upgrade, and there was previously no way to say no
* **security(eui):** the EUI socket is admitted against `SOLI_WS_MAX_CONNECTIONS` / `SOLI_WS_MAX_CONNECTIONS_PER_IP` like `/ws/*` (and so are LiveView and live-reload sockets); a client has 10 s to say Hello; the server pings every 30 s and closes after two unanswered; inbound frames are charged against the `/ws/*` message budget, a `Resync` twenty at a time; a client's `Error` message reaches the log with control bytes stripped; `EUI_TRACE` prints session state only under `--dev`
* **security(eui):** the `Welcome` frame named the session by the first sixteen bytes of the raw session cookie — the value `HttpOnly` keeps from page scripts. It carries a SHA-256 handle now, as the LiveView socket already did
* **security(eui):** an image/audio/video `src` must resolve under `public/` or `app/assets/`; any file under the app root could be read, hashed and served to anyone with the hash — the publisher key and `.env` included, whenever a view built a `src` from data
* **security(eui):** a duplicate key among siblings panicked the keyed diff, which restarted the realtime worker and reloaded the application — repeatable by any client whose row keys came from its own data. Duplicate keys are refused with a reason, the diff clamps its indices, and LiveView events run behind the same `catch_unwind` HTTP handlers have
* **security(eui):** the server enforces the client's protocol limits before sending — tree depth, node count, inline text length, and the atom/style/colour/chunk table ceilings — with a reason the view author can read, where the client used to refuse the batch silently (and a deep tree could run a worker off its stack)

### Performance

* **perf(eui):** each EUI session is pinned to one realtime worker (the default realtime share is now a quarter of the pool, was one thread), so the memo of kept subtrees — thread-local, it holds interpreter values — is hit every render instead of one in R; keyed rows inside a kept card stay warm; and the memo is actually freed when a session ends (it was cleared on the wrong thread, and every closed session's last tree stayed resident for the life of the process). A closed EUI session is detached and reaped after `DETACHED_GRACE` like a LiveView one; the reaper is started by the EUI upgrade too
* **perf(eui):** a slow reader no longer loses batches silently — a render of more than 32 frames, or a client a little behind, dropped whatever did not fit its queue and left the client with a tree the server had moved on from. The worker waits up to 2 s for room and then closes the socket, and the client resyncs. Error frames are sent when the session is really over: 503 (queue full), 504 (handler did not answer in 30 s), 500 (worker gone)
* **perf(eui):** `benches/eui_render.rs` — a 10k-row keyed list rendered six ways — is the encoder's regression net. Against it: a cold render reads the view's values directly (no per-node JSON map), styles are looked up by fingerprint, `local(…)` handlers compile once per session, atom names are O(1), an event resolves handler and props in one tree lookup off the reactor thread, batches are bounded by weight so any tree the client accepts can be mounted, the keyed reorder runs in C log C with removal runs coalesced, and the handler's returned state goes to the view as the value it is. Reversed list 56 → 27 ms, cleared 6.2 → 4.1 ms, values rebuilt 87 → 62 ms
* **perf(eui):** an asset GET is a refcount, not a copy; the store evicts least-recently-used instead of refusing forever at 256 MiB; the manifest is signed once per capability mask; the publisher key is created private
