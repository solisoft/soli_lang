...
* **feat(eui):** **live regions** (EUI 01 §2.7) — one corner of a page can be live while the rest stays a cached render nothing is held for. A node carrying `live`, an absolute path on the same origin, takes its *content* from a session of its own, so a documentation page with a comment count stops costing every reader a whole session. Its own children show until that session speaks, and they came from whatever rendered the page: an older client that ignores the prop, a session that cannot be opened and a page served from a cache therefore all end in the same place, content that is out of date rather than missing. Soli's half is a session address that may carry a query, arriving as `connect` params — `?for=1042` is how two regions of one component tell the application which of them is being rendered, percent-decoded, sixteen pairs, no nesting. **Nothing was added to the wire**: a live region is an ordinary session addressed by an ordinary prop, which is why an application can adopt one without its clients being rebuilt. The client half — translating a region's node ids into the page's space while each region keeps its own four interned tables — is not yet built
